...
2. **CycleGAN (Cycle-Consistent GAN)**
    ...
    Перенос визуального стиля между доменами **без пары изображений**.
...
- [Важные особенности](#important_features)
- [Запуск и настройка бота](#bot_start)
    - [Запуск в Docker](#bot_docker)
    - [Deploy](#bot_deploy)
- [Возможности и функционал бота](#bot_functionaliti)
- [Используемые материалы](#style_transfer)
    - [Стилизация изображений основана на алгоритме Гатиса(Neural Style Transfer - NST)](#style_transfer_gatys)
    - [Cycle-Consistent Generative Adversarial Networkr - CycleGAN](#style_transfer_cyclegan)
 - [TODO List](#bot_todo)
...
<a name="important_features"><h2>Важные особенности</h2></a>

* ✅ Асинхронная обработка запросов
* ✅ Поддержка режимов **polling** и **webhook**
* ✅ Использование `.env` файла для загрузки секретов
* ✅ Использование файлов конфигурации для загрузки параметров алгоритмов
...
* ✅ Настраиваемые параметры CPU/GPU/"auto", а также параметры алгоритмов
...
* ✅ Отсутствует функциональность обработки "сырого" HTTPS-трафика, поэтому для режима `webhook` - следует использовать шлюз, или реверс-прокси(или `ngrok`, `devtunnels` и т.п.)

<a name="bot_start"><h2>Запуск и настройка бота</h2></a>

**Важные замечания:** 
* Репозиторий не содержит модели(в виду их сравнительно большого размера), поэтому их нужно будет скачать.
* Для работы с моделями используется довольно "тяжелая" библиотека PyTorch, поэтому при установке зависимостей нужно быть готовым к скачиванию примерно **5.5 Гб**. Собираемые Docker-образы с приложением также будут занимать около **5.5 Гб**. 
...
*Для проверки работоспособности рекомендуется запустить бота в режиме `polling`, а для этого достаточно только одного параметра - `TELEGRAM_BOT_TOKEN`. Конфигурационные файлы так же изменять не рекомендуется.*
...
Сравнительно большие размеры моделей не позволили включить их в состав данного репозитория, поэтому пользователям предлагается самостоятельно их скачать из Интернета.  
...
* Для размещения в Интернет используется VPS(vCPU: 2, ОЗУ:2 Гб, SSD: 60 Гб) от AWS с бесплатным 90-дневным использованием. Без GPU(CUDA).
* Учитывая размер PyTorch и моделей, CI/CD построен так, что сборка выполняется на VPS(используется технология GitHub Self hosted runners). При этом, ранее скачанные модели монтируются в Docker. 
* Процесс описан в конфигурации: `.github/workflows/deploy-on-vps.yml`.
* Dockerfile
*
*
* В качестве обработчика "сырого" https-трафика на VPS развернут реверс-прокси Nginx(сгенерированы самоподписанные сертификаты).


<a name="bot_functionaliti"><h2>Возможности и функционал бота</h2></a>
...
<a name="style_transfer_gatys"><h3>Стилизация изображений основана на алгоритме Гатиса(Neural Style Transfer - NST)</h3></a>
...
<a name="style_transfer_cyclegan"><h3>Cycle-Consistent Generative Adversarial Networkr - CycleGAN</h3></a>
...
1. Таймауты во флоу(сессии "зависшие" в FSM реализованных флоу).
2. Очистка временных дирректорий при старте.
