...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "- Попробовать перенести стиль с одной картинки на другую (/nst)"
    )
...
async def cmd_help(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "/start - начать диалог\n"
        "/help - эта справка\n"
        "/nst - начать процесс Neural Style Transfer (пока в разработке)\n"
        "/cancel - отменить текущую операцию (например, загрузку картинок для /nst)\n"
